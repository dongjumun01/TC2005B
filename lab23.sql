...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 07, 2025 at 04:27 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `registrarMaterial` (IN `uClave` INT, IN `uDescripcion` VARCHAR(40), IN `uPrecio` FLOAT, IN `uImpuesto` FLOAT, IN `uPorcentajeImpuesto` FLOAT)   INSERT INTO materiales
VALUES (uClave, uDescripcion, uPrecio, uImpuesto, uPorcentajeImpuesto)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `registrarProyecto` (IN `uNumero` INT, IN `uDenominacion` VARCHAR(40))   INSERT INTO proyectos
VALUES (uNumero, uDenominacion)$$

--
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `consultaTotalEntregas` () RETURNS INT(11)  BEGIN
	DECLARE totalEntregas INT;
    SET totalEntregas = (SELECT SUM(cantidad) FROM entregan);
    RETURN totalEntregas;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `consultaTotalRegistros` () RETURNS INT(11)  BEGIN
	DECLARE total INT;
...
--
-- Stand-in structure for view `nombrevista`
-- (See below for the actual view)
--
CREATE TABLE `nombrevista` (
`clave` int(11)
...
--
-- Stand-in structure for view `vistamateriales2001`
-- (See below for the actual view)
--
CREATE TABLE `vistamateriales2001` (
`clave` int(11)
...
--
-- Structure for view `nombrevista`
--
DROP TABLE IF EXISTS `nombrevista`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `nombrevista`  AS SELECT `materiales`.`clave` AS `clave`, `materiales`.`descripcion` AS `descripcion` FROM `materiales` ;

-- --------------------------------------------------------

--
-- Structure for view `vistamateriales2001`
--
DROP TABLE IF EXISTS `vistamateriales2001`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vistamateriales2001`  AS SELECT `entregan`.`clave` AS `clave`, sum(`entregan`.`cantidad`) AS `total_vendido` FROM `entregan` WHERE year(`entregan`.`fecha`) = 2001 GROUP BY `entregan`.`clave` ;
